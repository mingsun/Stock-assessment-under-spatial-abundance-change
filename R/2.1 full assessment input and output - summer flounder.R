...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) {
    ...
    # BIG indices ratio
    load(paste0("manuscript/3. stock assessment with spatial abundance change/results/abundance indices/summer flounder/", scenario, "/BIG_ratio_FULL_", q,".Rdata"))
    ...
    WriteASAP3DatFile(paste0("manuscript/3. stock assessment with spatial abundance change/stock assessment/summer flounder/input data/ASAP3_MTA2023_FULL_",
...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) { 
    ...
    outdir <- paste0(assessment.path, "FULL_", scenario, "_", q)
    
    if (!dir.exists(outdir)) { 
      dir.create(outdir)
      
      file.copy(from = paste0(assessment.path, "input data/ASAP3_MTA2023_FULL_", scenario, "_", q ,".DAT"),
                to   = paste0(outdir, "/ASAP3_MTA2023_FULL_", scenario, "_", q ,".DAT"))
    }
...
# here we run retrospetive (5 years) to save effort
...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) {
    ...
    wd <- paste0(assessment.path, "FULL_", scenario , "_", q, "/")
    asap.name <- paste0("ASAP3_MTA2023_FULL_", scenario ,"_", q ,"_000")
    ...
    output <- read.csv(paste0(output.path, "ASAP_summary_ASAP3_MTA2023_FULL_", scenario , "_", q, "_000",".csv")) %>%
      add_column(Dataset = "FULL", Scenario = scenario, q = q) %>%
      write.csv(paste0(output.path, "ASAP_summary_ASAP3_MTA2023_FULL_", scenario , "_", q, ".csv"), row.names = FALSE)
    
    # delete the original output
    file.remove(paste0(output.path, "ASAP_summary_ASAP3_MTA2023_FULL_", scenario , "_", q, "_000",".csv"))
