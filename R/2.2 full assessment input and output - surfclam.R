...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) { 
    ...
    load(paste0("manuscript/3. stock assessment with spatial abundance change/results/abundance indices/surfclam/",
                scenario, "/MCDS_ratio_FULL_", q,".Rdata"))
    ...
    new_folder <- paste0("manuscript/3. stock assessment with spatial abundance change/stock assessment/surfclam/FULL_",
                         scenario, "_", q)
...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) { 
    ...
    run_dir <- paste0("manuscript/3. stock assessment with spatial abundance change/stock assessment/surfclam/FULL_",
                          scenario, "_", q)
    
    r4ss::run(dir = run_dir, skipfinished = TRUE) # skip any folders that already contain a "Report.sso" file
...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) {
    ...
    write.csv(SSB_df, paste0(output_path, "SSB_FULL_", scenario, "_", q, ".csv"))
    ...
    write.csv(F_df, paste0(output_path, "F_FULL_", scenario, "_", q, ".csv"))
    ...
    write.csv(ABC_df, paste0(output_path, "ABC_FULL_", scenario, "_", q, ".csv"), row.names = FALSE)
