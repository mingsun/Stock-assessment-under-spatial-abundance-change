...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) { 
    new_folder <- paste0("manuscript/3. stock assessment with spatial abundance change/stock assessment/squid/FULL_",
                         scenario, "_", q)
    ...
    file.copy(
      ...
      to   = file.path(new_folder, paste0("Index_script_FULL_", scenario, "_", q, ".xlsx"))
    )
    ...
    file.copy(
      ...
      to   = file.path(new_folder, paste0("Summary_FULL_", scenario, "_", q, ".csv"))
    )
...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) { 
    ...
    assessment_folder <- paste0("manuscript/3. stock assessment with spatial abundance change/stock assessment/squid/FULL_",
                                scenario, "_", q, "/")
    ...
    file.copy(
      from = paste0(assessment_folder, "Summary_FULL_", scenario, "_", q, ".csv"),
      to   = results_folder)
...
for (q in q.pattern) {
  ...
  for (scenario in c("S1", "S2")) { 
    ...
    read.csv(paste0(results_folder, "Summary_FULL_", scenario, "_", q, ".csv")) %>%
      add_column(Dataset = "FULL", .before = 1) %>%
      rename(Scenario = SCENARIO) %>%
      write.csv(paste0(results_folder, "Summary_FULL_", scenario, "_", q, ".csv"), row.names = FALSE)
